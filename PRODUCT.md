...
Engineers, engineering managers/recruiters, open-source collaborators, and
conference/program organizers evaluating Vandit Singh. They arrive from GitHub,
X/LinkedIn, search, or a shared link, usually scanning fast to answer one
question: *is this person a serious, credible systems engineer worth my time?*
Most are technical and skim-read; a meaningful share are on mobile.
...
a credible first impression in under five seconds and an easy path to "reach out".
...
- **Generic SaaS landing pages**: hero-metric templates, identical icon+heading
  card grids, gradient blobs, tiny uppercase tracked eyebrows above every section.
- **Loud / colorful**: bright multi-color palettes, neon, gradient text. Identity
  is monochrome; color is not the voice.
- **Over-animated**: parallax overload, scattered micro-interactions, motion for
  its own sake.
- **Cluttered / dense**: cramming, competing focal points, lost whitespace.
...
Target **WCAG 2.2 AA**. Body text ≥4.5:1, large/bold text ≥3:1, in both light
and dark themes. Meaning never carried by color alone (the monochrome system
helps here). Full keyboard operability with visible focus states. All motion
has a `prefers-reduced-motion` fallback (already implemented in `motion.css`);
reveal animations enhance already-visible content rather than gating it.
